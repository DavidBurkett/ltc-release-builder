...
def setup():
    ...
    # TODO: Find a better way to download dependency sources
    #if not os.path.isdir('litecoin'):
    #    subprocess.check_call(['git', 'clone', args.url])# 'https://github.com/litecoin-project/litecoin.git'])
...
def setup_mac_sdk():
    ...
    os.chdir(workdir)
    os.makedirs(os.path.join(workdir, 'gitian-builder', 'inputs'), exist_ok=True)
    if args.macos and not os.path.isfile('gitian-builder/inputs/{0}'.format(MAC_SDK)):
        if os.path.isfile(MAC_SDK):
            subprocess.check_call('cp sdk/{0} gitian-builder/inputs/{0}'.format(MAC_SDK), shell=True)
        else:
